...
//        MongoCollection es una interfaz genérica: 
			...
			// y es el tipo predeterminado para devolver la búsqueda (.find) y agregados
			// (.aggregate).
			// El método de un solo argumento getCollection devuelve una instancia de
...
//        y así es como podemos trabajar con instancias de la clase de documento.
			...
			for (int i = 0; i < consulta.size(); i++) {
				...
				System.out.println(
						alumno.getString("nombre") + "\t" + alumno.get("telefono") + "\t" + alumno.get("curso"));
			}
...
//        insertarOne2(coleccion);
	...
	public static void insertarOne2(MongoCollection<Document> coleccion) {
		...
		Document alumno2 = new Document("nombre", "Marisa").append("teléfono", 1234).append("curso",
	...
	public static void insertarMany(MongoCollection<Document> coleccion) {
		...
		// Si se desea saber los documentos de la colección se puede utilizar el método
		// count.
	...
	public static void consultarDocumentos_R(MongoCollection<Document> coleccion) {
		// Recuperar todos los documentos de la colección y visualizarlos en formato
		// Json:
		// creamos el cursor
...
//        Para actualizar un único documento se utiliza el método updateOne, 
		// si hay varios con nombre Ana, actualiza el primero. La actualización devuelve
		// un UpdateResult.
...
//        Añadir la población Madrid a Isabel.
		...
		// Eliminar el campo población a los alumnos de 1º DAM que lo tengan. Utilizamos
		// es método unset.
	...
	public static void filtrosConsultas(MongoCollection<Document> coleccion) {
		...
		// Buscar el document con la clave nombre iagual a Ana, devolver el primero.
		// Para ello
...
//        Recuperar los alumnos con nota superior a 5, visualizarlos en Json.
		...
		// creamos el cursor
		MongoCursor<Document> cursor2 = coleccion.find(Filters.or(Filters.eq("nota", 5), Filters.eq("nota", 8)))
				.iterator();
		// recorremos el cursor
		...
		// creamos el cursor
		MongoCursor<Document> cursor3 = coleccion
				.find(Filters.and(Filters.eq("nota", 8), Filters.eq("curso", "1º DAM"))).iterator();
		// recorremos el cursor
	...
	public static void usoProyecciones(MongoCollection<Document> coleccion) {
		...
		MongoCursor<Document> cursor1 = coleccion.find(eq("curso", "1ºDAM")).sort(ascending("nombre"))
...
//        Para la alumna Ana no mostrar el _id, el nombre y la nota
		...
		Document alumno = coleccion.find(eq("nombre", "Ana")).projection(exclude("_id", "nombre", "nota")).first();
		System.out.println(alumno.toJson());

		// Obtener el nombre y la nota de los 3 primeros alumnos de 1º DAM ordenados por
		// nombre
		MongoCursor<Document> cursor2 = coleccion.find(eq("curso", "1ºDAM")).sort(ascending("nombre"))
	...
	public static void usoAgregaciones(MongoCollection<Document> coleccion) {
		...
		// Obtener el nombre y la nota de los alumnos de 1º DAM, utilizando las etapas
		// match y project
		...
		System.out
				.println("Obtener el nombre y la nota de los alumnos de 1º DAM, utilizando las etapas match y project");
		// recorremos el cursor
		while (cursor1.hasNext()) {
			Document alumno = cursor1.next();
			System.out.println(alumno.toJson());
...
//        El primer parámetro del grupo lo dejamos vacío.
		// creamos nuestras operaciones de grupo, como no hay campo de agrupacion
		// ponemos ""
		// añadimos las operaciones
		Document operac2 = new Document("_id", "").append("total", new Document("$sum", "$nota")).append("media",
				new Document("$avg", "$nota"));
		...
		/// Uso Etapa $out
		// Si queremos guardar el resultado de la consulta en una nueva colección en la
		/// base de datos añadimos la etapa out.
		// creamos nuestras operaciones de grupo, primero el campo por el que
		// agrupamos y le añadimos las operaciones
		Document operac3 = new Document("_id", "$curso").append("total", new Document("$sum", "$nota")).append("media",
				new Document("$avg", "$nota"));
		// creamos el documento con $group
